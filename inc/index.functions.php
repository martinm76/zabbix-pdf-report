...
function ListOldReports(string $dir): void
{
    ...
    // Build a case-insensitive lookup set of all known host and group names.
    // Using array keys gives us O(1) membership tests via isset().
    $hostsFlat  = is_array($hosts ?? null)       ? array_flatten($hosts)       : [];
    $groupsFlat = is_array($host_groups ?? null) ? array_flatten($host_groups) : [];

    $known = [];
    foreach ($hostsFlat as $h) {
        $known[mb_strtolower((string) $h)] = true;
    }
    foreach ($groupsFlat as $g) {
